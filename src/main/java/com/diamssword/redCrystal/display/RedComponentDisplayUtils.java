...
public class RedComponentDisplayUtils {
	...
	public static TransformComponent getCenteredTransform(Vector3i position, BlockFace face, Vector2d offset) {
		var rot = FacingUtil.facingToRotation(face);
		return new TransformComponent(getCenteredPosition(position, face, offset), new Rotation3f(rot.x, rot.y, rot.z));
	}

	public static Vector3d getCenteredPosition(Vector3i position, BlockFace face, Vector2d offset) {
		return new Vector3d(position.x + 0.5, position.y + 0.5, position.z + 0.5).add(FacingUtil.facingToDir(face, 0.51, offset.x, offset.y));
	}
	...
	public static DisplayDebug getBeamPacket(@Nonnull Vector3d position, @Nonnull Vector3d direction, @Nonnull Vector3f color, float time, float scale, boolean fade) {
		...
		matrix.rotateY(angleY + (Math.PI / 2));
		...
		return new DisplayDebug(DebugShape.Cylinder, matrixData, color, time, buildFlags(fade), null, 0.8f);
	...
	public static void drawBeam(Store<EntityStore> store, Vector3d from, Vector3d to, Ref<EntityStore> player) {
		...
		Vector3d relativeTargetOffset = new Vector3d(from.x - to.x, from.y - to.y, from.z - to.z).negate();
		float yaw = (float) Math.atan2(relativeTargetOffset.z, relativeTargetOffset.x);
		float pitch = -(float) Math.atan2(Math.sqrt(relativeTargetOffset.x * relativeTargetOffset.x + relativeTargetOffset.z * relativeTargetOffset.z), relativeTargetOffset.y);

		ParticleUtil.spawnParticleEffect("Test", from.x, from.y, from.z, (float) (Math.toRadians(90) + yaw), pitch, 0f, 0.1f, null, null, List.of(player), store);
	}
}
