...
public class LaserDetectorBehavior extends RedCompBehaviorWithSettings<BehaviorAsset, LaserDetectorBehavior.LaserDetectorSettings> {
	...
	public void displayTick() {
		...
		if(getSettings().showLaser) {
			...
			if(main != null && main.isValid()) {
				...
				RedComponentDisplayUtils.drawLaserFor(main.getStore(), base, new org.joml.Vector3d(rot).add(base), 0.6f, active ? LinkingState.ERROR_BEAM_COLOR : LinkingState.BASE_BEAM_COLOR, 0.1f);
			}
		}
	}
	...
	public List<Ref<EntityStore>> getEntities() {
		...
		var p1 = new org.joml.Vector3d(base);
		var p2 = new org.joml.Vector3d(base).add(rot);
