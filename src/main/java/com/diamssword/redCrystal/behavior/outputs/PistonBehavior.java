...
public class PistonBehavior extends RedCompBehaviorWithSettings<BehaviorAsset, PistonBehavior.PistonSettings> {
	...
	public BlockSelection getSelection(boolean retracting) {
		...
		if(size > 0 && (retracting || size <= this.getSettings().selectionLength)) {
			...
			var basePos = new Vector3d(pos.x, pos.y, pos.z);
			var a = FacingUtil.facingToDir(FacingUtil.opposite(this.parent.getFace()), 1 + getInternalState("extended"), 0, 0).add(basePos);
			var b = FacingUtil.facingToDir(FacingUtil.opposite(this.parent.getFace()), getInternalState("extended") + size, 0, 0).add(basePos);
			selec.setSelectionArea(Vector3dUtil.toVector3i(a), Vector3dUtil.toVector3i(b));
			return selec;
		}
		return null;
	}
	...
	public int getMaxSize() {
		...
		var vec1 = FacingUtil.facingToDir(FacingUtil.opposite(this.parent.getFace()), 1 + getInternalState("extended"), 0, 0).add(new Vector3d(pos.x, pos.y, pos.z));
		var size = this.getSettings().selectionLength + 1;
		for(int i = 0; i < this.getSettings().selectionLength + 1; i++) {
			var p1 = Vector3dUtil.toVector3i(new Vector3d(vec1).add(new Vector3d(vec).mul(i)));
			if(!canMove(p1)) {
				size = -1;
				break;
			}
	...
	public void moveBlock(boolean retract) {
		...
		var pos = this.parent.getParent().getPosition();
		if(retract) {
			if(getWorld().getBlock(Vector3dUtil.toVector3i(FacingUtil.facingToDir(FacingUtil.opposite(this.parent.getFace()), getInternalState("extended"), 0, 0).add(new Vector3d(pos.x, pos.y, pos.z)))) == 0) {
				move(true, Vector3dUtil.toVector3i(FacingUtil.facingToDir(FacingUtil.opposite(this.parent.getFace()), retract ? -1 : 1, 0, 0)), false, false, getWorld().getEntityStore().getStore());
				playSound();
			}
			...
			if(getMaxSize() <= this.getSettings().selectionLength) {
				move(false, Vector3dUtil.toVector3i(FacingUtil.facingToDir(FacingUtil.opposite(this.parent.getFace()), retract ? -1 : 1, 0, 0)), false, false, getWorld().getEntityStore().getStore());
				setInternalState("extended", (short) (getInternalState("extended") + (retract ? -1 : 1)));
				playSound();
			}
		}
	...
	public boolean canMove(Vector3i pos) {
		...
		var bb = BlockBoundingBoxes.getAssetMap().getAsset(block.getHitboxTypeIndex());
	...
	public void playSound() {
		var position = parent.getParent().getPosition();
		var block = getWorld().getBlockType(new Vector3i(position.x, position.y, position.z));
		...
		if(seatSoundId != null) {
			...
			SoundUtil.playSoundEvent3d(soundEventIndex, SoundCategory.SFX, new Vector3d(position.x + 0.5, position.y + 0.5, position.z + 0.5), getWorld().getEntityStore().getStore());
		}
	...
	public void move(boolean retracting, @Nonnull Vector3i direction, boolean empty, boolean entities, @Nonnull ComponentAccessor<EntityStore> componentAccessor
		...
		Vector3i min = new Vector3i(
				Math.min(selectionMin.x, selectionMax.x),
				Math.min(selectionMin.y, selectionMax.y),
				Math.min(selectionMin.z, selectionMax.z)
		);
		Vector3i max = new Vector3i(
				Math.max(selectionMin.x, selectionMax.x),
				Math.max(selectionMin.y, selectionMax.y),
				Math.max(selectionMin.z, selectionMax.z)
		);
		int xMin = min.x;
		int xMax = max.x;
		int yMin = min.y;
		...
		if(entities) {
			for(Ref<EntityStore> targetEntityRef : TargetUtil.getAllEntitiesInBox(new Vector3d(min.x, min.y, min.z), new Vector3d(max.x, max.y, max.z), componentAccessor)) {
		...
		ClipboardBoundsSnapshot clipboardSnapshot = new ClipboardBoundsSnapshot(selection);
		Vector3i destMin = new Vector3i(min).add(direction);
		Vector3i destMax = new Vector3i(max).add(direction);
		beforePlace.setSelectionArea(
				new Vector3i(Math.min(min.x, destMin.x), Math.min(min.y, destMin.y), Math.min(min.z, destMin.z)),
				new Vector3i(Math.max(max.x, destMax.x), Math.max(max.y, destMax.y), Math.max(max.z, destMax.z))
		);
